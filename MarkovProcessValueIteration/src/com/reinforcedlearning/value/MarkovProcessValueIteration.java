...
public class MarkovProcessValueIteration {
    ...
    public MarkovProcessValueIteration() {
    	...
    	pi = new char[Constants.NUM_ROWS][Constants.NUM_COLUMNS]; 
    	//the V value function values
    	v = new double[Constants.NUM_ROWS][Constants.NUM_COLUMNS];
    	//initialize V'
        vNext = new double[Constants.NUM_ROWS][Constants.NUM_COLUMNS];
        //reward function R(s',s)
        r = new double[Constants.NUM_ROWS][Constants.NUM_COLUMNS];
    ...
    private void initializeVariables() {
    	...
    	for (int rowIndex=0; rowIndex<Constants.NUM_ROWS; rowIndex++) {
            for (int columnIndex=0; columnIndex<Constants.NUM_COLUMNS; columnIndex++) {
                r[rowIndex][columnIndex] = Constants.STATE_REWARD;
            }
        }
    ...
    public double goDown(int row, int col) {
    	...
    	//if can not: stay at the same state
        if ((row==Constants.NUM_ROWS-1) || (row==0 && col==1))
            return v[row][col];
    ...
    public double goRight(int row, int col) {
    	...
    	//if can not: stay at the same state
        if ((col==Constants.NUM_COLUMNS-1) || (row==1 && col==0))
            return v[row][col];
